# Experiment Summary: Finance Feed Agent + Consumer Agent with Data Facts and A2A Communication
...
### 2. Consumer Agent (`examples/data_consumer_agent.py`)

**Purpose:** Discovers agents, reads Data Facts, and accesses datasets + A2A communication
...
**Results:**
```
Step 1: Consumer Agent Uses Direct Data Facts URL
[OK] Using direct Data Facts URL
...
[OK] FRESH (Age: 2.7 seconds, TTL: 600 seconds)
...
- ✅ Consumer Agent queries Finance Feed Agent
- ✅ Finance Feed Agent responds via A2A
- ✅ Response contains relevant stock price information
...
Test 2.2: Asking Consumer Agent 'what are the current stock prices?'
[OK] Consumer Agent handled stock price query
...
Query: 'What are the current stock prices for TSLA, AAPL, and ETH-USD?'
...
### Documentation Files
- `agents/agent_finance_feed_01/EXPERIMENT_SUMMARY.md` - This document
...
Finance Feed Agent
  ↓
NANDA Adapter (data_facts_url parameter)
  ↓
RegistryClient.register_agent()
  ↓
Registry (stores data_facts_url in Agent Facts)
```
...
### Data Facts Validation

1. **Access Type Check**: Validates `access_type == "public"`
2. **Freshness Check**: Compares `last_updated + ttl_seconds` with current time
3. **Checksum Verification**: Computes SHA256 checksum of dataset and compares with Data Facts evidence
...
- Real-time query-response pattern
- Fallback mechanisms for robustness
- Standard NEST A2A protocol
...
The Finance Feed Agent and Consumer Agent are fully integrated and working correctly with both Data Facts and A2A communication!
